...
// Mapa de páginas a sus fragmentos HTML
const PAGE_PATHS = {
...
function _loadPageThen(p, callback) {
    // Si ya está en el DOM, ir directo
    const existing = document.getElementById('page-' + p);
    if (existing) { callback(existing); return; }

    // Si ya se está cargando, esperar
    if (_loadedPages[p] === 'loading') {
        setTimeout(() => _loadPageThen(p, callback), 50);
        return;
    }
    ...
    _loadedPages[p] = 'loading';
    fetch(path)
        .then(r => r.text())
        .then(html => {
            const container = document.getElementById('pages-container');
            container.insertAdjacentHTML('beforeend', html);
            _loadedPages[p] = 'loaded';
            const el = document.getElementById('page-' + p);
            if (el) callback(el);
            else console.warn('goPage: page-' + p + ' no encontrado en fragmento');
        })
...
// Arranque: cargar home primero, luego inicializar
