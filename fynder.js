...
// Tiempo de expiración simulado por oferta (horas desde carga)
const OFFER_EXPIRY = {
  "1":  { h: 23, m: 47 },
  "2":  { h: 11, m: 30 },
  "10": { h: 47, m: 15 },
...
function renderOffers() {
  ...
  const deals = BUSINESSES.filter(b => b.deal);

  if (deals.length === 0) {
    grid.innerHTML = '<p style="color:var(--muted);font-size:.9rem">No hay ofertas activas en este momento.</p>';
    return;
  }

  grid.innerHTML = deals.map(b => {
    const expiry = OFFER_EXPIRY[b.id] || { h: 24, m: 0 };
    const catObj = CATEGORIES.find(c => c.id === b.categoryId);
    const isFav  = favorites.has(b.id);

    return `
    <div class="offer-card" onclick="openModal('${b.id}')">
      <!-- Imagen -->
      <div class="offer-img-wrap">
        <img src="${b.image}" alt="${b.name}" class="offer-img" loading="lazy">
        <!-- Badge de descuento -->
        <div class="offer-deal-badge" style="background:${b.deal.color}">
          <i class="fas fa-tags"></i> ${b.deal.label}
        </div>
        <!-- Favorito -->
        <button class="offer-fav-btn ${isFav ? 'active' : ''}"
          onclick="event.stopPropagation();toggleFav('${b.id}');this.classList.toggle('active')"
          title="${isFav ? 'Quitar de guardados' : 'Guardar'}">
          <i class="fas fa-heart"></i>
        </button>
        <!-- Countdown -->
        <div class="offer-countdown" id="offerCountdown_${b.id}">
          <i class="fas fa-clock"></i>
          <span class="offer-countdown-time" data-h="${expiry.h}" data-m="${expiry.m}">
            ${String(expiry.h).padStart(2,'0')}:${String(expiry.m).padStart(2,'0')}
          </span>
        </div>
      </div>
      ...
      <div class="offer-body">
        <div class="offer-cat-row">
          ${catObj ? `<span class="offer-cat" style="color:${catObj.color};background:${catObj.bg}">${catObj.label}</span>` : ''}
          <span class="offer-rating"><i class="fas fa-star"></i> ${b.rating}</span>
        </div>
        <h3 class="offer-name">${b.name}</h3>
        <p class="offer-deal-desc"><i class="fas fa-tag" style="color:${b.deal.color}"></i> ${b.deal.desc}</p>
        <div class="offer-footer">
          <span class="offer-address"><i class="fas fa-location-dot"></i> ${b.address.split(',')[0]}</span>
          <button class="offer-cta" onclick="event.stopPropagation();openModal('${b.id}')">
...
function startOfferCountdowns() {
  // Actualiza cada segundo los timers de las ofertas
  clearInterval(window._offerTimer);
  window._offerTimer = setInterval(() => {
    document.querySelectorAll('.offer-countdown-time').forEach(el => {
      let h = parseInt(el.dataset.h) || 0;
      let m = parseInt(el.dataset.m) || 0;
      let s = parseInt(el.dataset.s || '59');
      ...
      // Cambia a rojo cuando queda menos de 1 hora
      const parent = el.closest('.offer-countdown');
      if (parent) parent.classList.toggle('urgent', h < 1);
    });
  }, 1000);
}
